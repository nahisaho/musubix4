...
After the work, run `npx musubix4 workflow-record sdd-knowledge complete --status
completed` exactly once.

1. Run `npx musubix4 knowledge build`. Markdown artifacts and the last 100 Git
   commits are indexed locally in `.musubix/cache/knowledge.json`.
2. Query `npx musubix4 knowledge query "your requirement or decision" --json`.
