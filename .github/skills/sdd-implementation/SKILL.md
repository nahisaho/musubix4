...
Run only the repository's exact `musubix4` CLI. Never fall back to similarly
...
2. For behavior changes and bug fixes, write a meaningful failing test before
   ...
   `npx musubix4 tdd red <TEST-ID> --requirement <REQ-ID> --command <name>`.
...
5. Before recording Red, run every configured `tdd.redPreflightCommands`
   formatter/check and let musubix4 enforce that preflight. Then run the
   ...
   `npx musubix4 mutation doctor --json` to inspect locally available engines
...
6. Regenerate `npx musubix4 trace build`, check `trace check --strict`, then
   `npx musubix4 gate --changed`. Configure real command/argument arrays first.
