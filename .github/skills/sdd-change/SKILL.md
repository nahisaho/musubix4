...
Record exactly one final invocation outcome with `npx musubix4 workflow-record sdd-change complete --status <status>`; `change-record` separately proves phases.
...
2. Run `npx musubix4 tdd red <TEST-ID> --requirement <REQ-ID> --command <name>`.
