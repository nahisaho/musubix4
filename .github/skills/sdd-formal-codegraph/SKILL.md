...
After the work, run `npx musubix4 workflow-record sdd-formal-codegraph complete
--status completed` exactly once.

## Formal consistency / 形式的整合性
1. Run `npx musubix4 formal generate <requirements-file> --format both --json`
   to inspect reproducible SMT-LIB2 and Lean artifacts before execution.
2. Run `npx musubix4 formal doctor --json`, then
   `npx musubix4 formal check <requirements-file> --solver none --json`.
...
## Code impact / コード影響
1. Run `npx musubix4 graph index` (or `graph index --changed`).
