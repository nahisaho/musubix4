...
try {
  ...
  writeFileSync(resolve(consumer, 'package.json'), '{"name":"musubix4-smoke-consumer","private":true,"type":"module"}\n');
  execFileSync(process.execPath, [
    npmCli, 'install', '--ignore-scripts', '--no-audit', '--no-fund', '--workspaces=false', resolve(work, packed.filename),
  ], { cwd: consumer, stdio: 'pipe' });
  const executable = resolve(consumer, 'node_modules/musubix4/dist/packages/cli/src/main.js');
  ...
  execFileSync(process.execPath, ['--input-type=module', '-e', "const {validateRequirements}=await import('musubix4/domain'); if (!validateRequirements('## REQ-SMOKE-001: Smoke\\nStatement: The system shall respond.').valid) process.exit(1);"], { cwd: consumer, stdio: 'pipe' });
  execFileSync(process.execPath, ['--input-type=module', '-e', "const {githubOidcAudience}=await import('musubix4/attestation'); if (!githubOidcAudience('urn:smoke','key-id','key').includes('musubix4-key-id=key')) process.exit(1);"], { cwd: consumer, stdio: 'pipe' });
  const plugin = JSON.parse(readFileSync(resolve(consumer, 'node_modules/musubix4/plugin.json'), 'utf8'));
