  ...
  for (const change of evidence.changes) {
    ...
    for (const requirementId of change.requirementIds) {
      ...
      for (const node of trace.nodes.filter((candidate) => candidate.kind === 'test')) {
        ...
        const annotationBlocks = source.match(/\/\*\*[\s\S]*?\*\//g) ?? [];
        if (annotationBlocks.some((block) =>
          new RegExp(`@id\\s+${node.id}\\b`).test(block)
          && new RegExp(`@verifies\\s+[^\\r\\n]*\\b${requirementId}\\b`).test(block))) {
