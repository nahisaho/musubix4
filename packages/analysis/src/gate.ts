...
import {
  ...
  writeModelCorrespondenceEvidence,
} from "./model-correspondence.js";
import {
  verifyEvidenceAttestation,
  type AttestationVerificationOptions,
} from "./attestation.js";
import {
  domainOwning,
  domainsConfigured,
  resolveDomains,
  validateApprovals,
  validateApprovalsForFeatureGate,
...
export async function runGate(
  ...
  const hasChangeDocuments = paths.some((path) =>
    /^\.musubix\/changes\/CHANGE-\d+\.md$/.test(path),
  );
  const featureDir = options.feature
    ? `.musubix/features/${options.feature}/`
    : null;
  if (featureDir && !paths.some((p) => p.startsWith(featureDir))) {
    throw new Error(
      `Unknown feature "${options.feature}"; no artifacts found under ${featureDir}.`,
    );
  }
  const checks: Evidence[] = [];
  const required = (name: string): boolean =>
    config.requiredChecks.includes(name);
  const countErrors = (diagnostics: Diagnostic[]): number =>
  ...
  function add(
    ...
    checks.push({
      ...
      summary: !present
        ? "No applicable artifact/input; not evaluated."
        : `${countErrors(diagnostics)} error(s).`,
      diagnostics,
    });
  }
  const requirementPaths = paths.filter((p) =>
    /^\.musubix\/features\/[^/]+\/requirements\.md$/.test(p),
  );
  const designPaths = paths.filter((p) =>
    /^\.musubix\/features\/[^/]+\/design\.md$/.test(p),
  );
  const requirements = await Promise.all(
    requirementPaths.map(async (p) =>
      validateRequirements(await readText(root, p), p),
    ),
  );
  const requirementIds = new Set(
    requirements.flatMap((r) => r.value.map((req) => req.id)),
  );
  const adrIds = new Set(
    paths
      .filter((p) => /^\.musubix\/decisions\/ADR-\d+\.md$/.test(p))
      .map((p) => p.split("/").at(-1)!.replace(/\.md$/, "")),
  );
  const designTexts = await Promise.all(
    designPaths.map(async (path) => ({
      path,
      text: await readText(root, path),
    })),
  );
  const designIds = new Set(
    designTexts.flatMap(({ text }) =>
      validateDesign(text).value.map((c) => c.id),
    ),
  );
  const designs = designTexts.map(({ path, text }) =>
    validateDesign(text, path, { requirementIds, adrIds, designIds }),
  );
  ...
  const featureRequirementIds = featureDir
    ? new Set(
        requirements
          .filter((_, i) => requirementPaths[i]!.startsWith(featureDir))
          .flatMap((r) => r.value.map((req) => req.id)),
      )
    : null;
  const featureDesignIds = featureDir
    ? new Set(
        designs
          .filter((_, i) => designPaths[i]!.startsWith(featureDir))
          .flatMap((d) => d.value.map((c) => c.id)),
      )
    : null;
  ...
  const policy = policyBaseline
    ? policyDiagnostics(config, policyBaseline, changed)
    : [];
  checks.push({
    name: "policy",
    ...
    status:
      policyBaseline === null
        ? "skipped"
        : policy.some((diagnostic) => diagnostic.severity === "error")
          ? "fail"
          : "pass",
    summary:
      policyBaseline === null
        ? "No trusted policy baseline is configured."
        : `${policy.length} policy violation(s).`,
    diagnostics: policy,
  });
  add(
    "requirements",
    requirementPaths.length > 0,
    reqDiagnostics.filter(scopedToFeature),
  );
  add(
    "design",
    designPaths.length > 0,
    designDiagnostics.filter(scopedToFeature),
  );
  const constitutionPath = ".musubix/constitution.md";
  const constitution = (await exists(within(root, constitutionPath)))
    ? validateConstitution(
        await readText(root, constitutionPath),
        constitutionPath,
      )
    : null;
  ...
  add(
    "trace",
    requirementPaths.length > 0,
    traceResult.diagnostics.filter(scopedToFeature),
  );
  ...
  const formalText = (
    await Promise.all(
      requirementPaths.map(async (path) =>
        (await readText(root, path)).replace(/^---[\s\S]*?---\s*/, ""),
      ),
    )
  ).join("\n\n");
  const formalResult = await formalCheck(
    formalText,
    root,
    {
  ...
  const modeledFraction = totalRequirements
    ? modeledRequirements / totalRequirements
    : 0;
  ...
  const formalRequired = required("formal");
  const formalHasErrors = formalDiagnostics.some(
    (diagnostic) => diagnostic.severity === "error",
  );
  checks.push({
    name: "formal",
    required: formalRequired,
    status: !requirementPaths.length
      ? "skipped"
      : !formalHasErrors && (formalResult.valid || !formalRequired)
        ? "pass"
        : "fail",
   ...
   * @design DES-AUTONOMOUS-DEVELOPMENT-001
   */
  const { workflowWaivers, workflowWaiverDiagnostics } =
    deriveWorkflowWaiverAudit(workflow.workflowWaiverContext);
  ...
  checks.push({
    ...
    status: !tdd.present
      ? "skipped"
      : (featureDir ? countErrors(tddDiagnostics) === 0 : tdd.valid)
        ? "pass"
        : "fail",
    summary: tdd.present
      ? `${tdd.cycles} Red-Green TDD cycle(s) recorded.`
      : "No TDD cycle evidence is available.",
  ...
  checks.push({
    ...
    status: !changes.present
      ? "skipped"
      : (featureDir ? countErrors(changeDiagnostics) === 0 : changes.valid)
        ? "pass"
        : "fail",
    summary: changes.present
      ? `${changes.changes} staged change(s) checked for ordered artifact and TDD evidence.`
      : "No staged change chronology evidence is available.",
    diagnostics: changeDiagnostics,
  });
  const completeness = await validateChangeCompleteness(root);
  const completenessDiagnostics =
    completeness.diagnostics.filter(scopedToFeature);
  const completenessCheck: Evidence = {
    name: "change-completeness",
    required: required("change-completeness") || hasChangeDocuments,
    status: !completeness.present
      ? "skipped"
      : (
            featureDir
              ? countErrors(completenessDiagnostics) === 0
              : completeness.valid
          )
        ? "pass"
        : "fail",
  ...
  const structuredTests = new Map<
    string,
    MusubixTestReport["tests"][number][]
  >();
  const structuredTestPaths = new Map<string, Set<string>>();
  ...
  for (const [commandIndex, command] of config.commands.entries()) {
    ...
    const args = command.adapter
      ...
      : configuredArgs;
    const result = await runner(command.command, args, {
    ...
    if (reportPath && result.status === "completed" && result.exitCode === 0) {
      const reportText = adapterOutput
        ? await readAdapterOutput(
            adapterOutput,
            await safePath(root, reportPath),
            result.stdout,
          )
        : (await exists(within(root, reportPath)))
          ? await readText(root, reportPath)
          : null;
      if (reportText === null) {
        const diagnostic: Diagnostic = {
          code: command.mutationReport
        ...
        try {
          ...
          const skippedTests = report.tests.filter(
            (test) => test.status === "skipped",
          );
          const unsuccessfulTests = report.tests.filter(
            (test) => test.status === "failed" || test.status === "error",
          );
          const executedTests = report.tests.filter(
            (test) => test.status !== "skipped",
          );
          const executionDiagnostics: Diagnostic[] = [];
          if (command.required && command.testReport) {
            executionDiagnostics.push(
              ...report.tests
                .filter((test) => !report.testFiles?.[test.id])
                .map((test) => ({
                  code: "TEST_REPORT_PATH_UNBOUND",
                  severity: "error" as const,
                  message: `${test.id} has no authoritative source path binding.`,
                  path: reportPath,
                })),
            );
          }
          if (!executedTests.length) {
            executionDiagnostics.push({
              code: skippedTests.length
                ? "TEST_REPORT_ALL_SKIPPED"
                : "TEST_REPORT_NO_EXECUTED_TESTS",
              severity: "error",
          ...
          if (executionDiagnostics.length) {
            ...
            if (command.required)
              testReportDiagnostics.push(...executionDiagnostics);
          } else {
            const sourceKind = adapterOutput?.source ?? "file";
            performanceExecutions.push(
              createPerformanceExecution({
                runId: gateRunId,
                executionId: digest(
                  JSON.stringify({
            ...
            for (const test of report.tests) {
              structuredTests.set(test.id, [
                ...(structuredTests.get(test.id) ?? []),
                test,
              ]);
              const reportedPath = report.testFiles?.[test.id];
              if (reportedPath) {
                const paths =
                  structuredTestPaths.get(test.id) ?? new Set<string>();
                paths.add(reportedPath.replaceAll("\\", "/"));
                structuredTestPaths.set(test.id, paths);
              }
            }
          }
        } catch (cause) {
          ...
          if (command.mutationReport)
            mutationReportDiagnostics.push(diagnostic);
          else if (command.required) testReportDiagnostics.push(diagnostic);
          commandCheck.status = "fail";
          commandCheck.diagnostics = [
            ...(commandCheck.diagnostics ?? []),
  ...
  const annotatedTestIds = trace.nodes
    .filter((node) => node.kind === "test")
    .map((node) => node.id);
  const executedTestIds = trace.nodes
    .filter(
      (node) =>
        node.kind === "test" &&
        structuredTests
          ...
          ?.some(
            (test) =>
              test.status === "passed" &&
              (!structuredTestPaths.has(node.id) ||
                structuredTestPaths
                  .get(node.id)
                  ?.has(node.path.replaceAll("\\", "/"))),
          ),
    )
    .map((node) => node.id);
  const pathDiagnostics = trace.nodes
    .filter(
      (node) =>
        node.kind === "test" &&
        structuredTestPaths.has(node.id) &&
        !structuredTestPaths.get(node.id)?.has(node.path.replaceAll("\\", "/")),
    )
    .map((node) => ({
      code: "TEST_REPORT_PATH_MISMATCH",
      severity: "error" as const,
      message: `${node.id} passed only from a reported path that does not match ${node.path}.`,
      path: node.path,
    }));
  const identityDiagnostics = [
    ...
    ...pathDiagnostics,
    ...annotatedTestIds
      .filter((id) => !executedTestIds.includes(id))
      .map((id) => ({
        code: "TEST_ID_NOT_PASSED",
        severity: "error" as const,
  ...
  const identitiesPresent =
    annotatedTestIds.length > 0 && configuredTestReports > 0;
  checks.push({
  ...
  checks.push({
    ...
    required:
      required("mutation") ||
      (config.mutation.mode === "strict" && mutation.requirements > 0) ||
      mutation.present,
    status: !mutation.present
      ? mutation.valid
        ? "skipped"
        : "fail"
      : mutation.valid
  ...
  await writeModelCorrespondenceEvidence(
    root,
    trace,
    formalEvidence,
  ...
  checks.push({
    ...
    required:
      required("model-correspondence") || correspondence.requirements > 0,
    status:
      correspondence.requirements === 0 && !correspondence.present
        ? "skipped"
        : correspondence.valid
          ? "pass"
          : "fail",
    summary:
      correspondence.requirements === 0
  ...
  const refreshedCompletenessDiagnostics =
    refreshedCompleteness.diagnostics.filter(scopedToFeature);
  completenessCheck.status = !refreshedCompleteness.present
    ? "skipped"
    : (
          featureDir
            ? countErrors(refreshedCompletenessDiagnostics) === 0
            : refreshedCompleteness.valid
        )
      ? "pass"
      : "fail";
  ...
  checks.push({
    ...
    required:
      ...
      config.attestation.mode === "ci-required" ||
      attestation.present,
    status:
      attestation.status === "off" || attestation.status === "unsigned-local"
        ? "skipped"
        : attestation.valid
          ? "pass"
          : "fail",
  ...
  if (featureDir && domainsOn) {
    ...
    if (!owningDomain)
      throw new Error(
        `Feature "${options.feature}" is not owned by any configured approval domain.`,
      );
  ...
  const approvalStages = domainsOn
    ? [
        ...(approvals.domains?.flatMap((d) => d.stages) ?? []),
        ...(approvals.release ? [approvals.release] : []),
      ]
    : approvals.stages;
  checks.push({
    name: "approval",
    required:
      ...
      approvals.present,
    status:
      !approvals.present && config.approval.mode === "compatible"
        ? "skipped"
        : approvals.valid
          ? "pass"
          : "fail",
  ...
  checks.push({
    name: "commands",
    required: required("commands"),
    status: !commandChecks.length
      ? "skipped"
      : aggregateStatus(commandChecks) === "fail"
        ? "fail"
        : "pass",
    summary: commandChecks.length
      ? `${commandChecks.length} configured command(s) executed; optional failures are nonblocking.`
      : "No configured commands; build/test evidence is missing.",
  });
  const metrics: Record<string, number> = {
    ...
    "commands.failures": commandChecks.filter((c) => c.status === "fail")
      .length,
    "commands.skipped":
      commandChecks.filter((c) => c.status === "skipped").length +
      Number(!commandChecks.length),
    ...
    "changes.complete": refreshedCompleteness.changes.filter(
      (change) => change.valid,
    ).length,
    "changes.completenessErrors": countErrors(
      refreshedCompleteness.diagnostics,
    ),
    ...
    "modelCorrespondence.coveredRequirements":
      correspondence.coveredRequirements.length,
    ...
    ...(traceResult.coverage.design === null
      ? {}
      : { "coverage.design": traceResult.coverage.design }),
    ...(traceResult.coverage.implementation === null
      ? {}
      : { "coverage.implementation": traceResult.coverage.implementation }),
    ...(traceResult.coverage.tests === null
      ? {}
      : { "coverage.tests": traceResult.coverage.tests }),
  };
  if (constitution?.valid) {
    for (const rule of constitution.value.rules) {
      ...
      checks.push({
        name: `constitution:${rule.id}`,
        required: required("constitution"),
        status:
          measured === undefined || !evidence || evidence.status === "skipped"
            ? "skipped"
            : measured <= rule.limit
              ? "pass"
              : "fail",
  ...
  if (inputChanges.length) {
    checks.push({
      ...
      diagnostics: inputChanges.map((change) => ({
        ...
        path: change.path,
        message:
          `${change.path} was ${change.change} during gate execution` +
  ...
  for (const path of changed ?? []) {
    if (graph.files.includes(path))
      for (const item of graphImpact(graph, path)) impacted.add(item.path);
  }
  ...
  if (options.changed) {
    ...
    lastChangeAnalysis = {
      baseline: "HEAD",
      head:
        revision.status === "completed" && revision.exitCode === 0
  ...
  const featureScopedCheckNames = new Set([
    "requirements",
    "design",
    "trace",
    "tdd",
  ...
  const [waivers, changeWaiverDiagnostics] = await Promise.all([
    activeWaivers(root),
    waiverEvidenceDiagnostics(root),
  ]);
  const waiverDiagnostics = [
    ...changeWaiverDiagnostics,
  ...
  const report: GateReport = {
    ...
    impacted: options.changed
      ? currentImpacted
      : (lastChangeAnalysis?.impacted ?? []),
    lastChangeAnalysis,
    fingerprints: after,
    waivers,
    workflowWaivers,
...
export async function projectStatus(root: string): Promise<{
  ...
  const approvals = config
    ...
    : null;
  const artifacts = {
    requirements: paths.filter((p) =>
      /^\.musubix\/features\/[^/]+\/requirements\.md$/.test(p),
    ).length,
    designs: paths.filter((p) =>
      /^\.musubix\/features\/[^/]+\/design\.md$/.test(p),
    ).length,
    decisions: paths.filter((p) =>
      /^\.musubix\/decisions\/ADR-\d+\.md$/.test(p),
    ).length,
  };
  const evidencePath = ".musubix/evidence/quality.json";
  ...
  if (await exists(within(root, evidencePath))) {
    ...
    ) as Partial<GateReport>;
    if (
      evidence.schemaVersion !== 1 ||
      !["pass", "fail", "skipped"].includes(String(evidence.status))
    )
      throw new Error("Invalid quality evidence; run gate.");
    if (evidence.status === "pass" || evidence.status === "fail") {
      status =
        evidence.fingerprints &&
        JSON.stringify(evidence.fingerprints) ===
          JSON.stringify(await evidenceSnapshot(root))
          ? evidence.status
          : "stale";
      if (
        status === "pass" &&
        (!evidence.checks?.length ||
          aggregateStatus(evidence.checks) !== "pass")
      )
        status = "stale";
      if (status === "pass") {
        const performance = await validatePerformanceEvidence(root);
        ...
        const mutation = await validateMutationEvidence(root);
        if ((mutation.present || mutation.requirements > 0) && !mutation.valid)
          status = "stale";
        const correspondence = await validateModelCorrespondenceEvidence(root);
  ...
  return {
    ...
    gate: {
      status,
      generatedAt,
      ready: initialized && status === "pass" && approvals?.valid === true,
    },
    waivers,
    workflowWaivers,
    waiverDiagnostics,
    next: !initialized
      ...
      : approvals && !approvals.valid
        ? approvals.domains
          ? [
              ...approvals.domains.flatMap((d) =>
                d.stages
                  .filter(
                    (stage) =>
                      stage.status !== "approved" &&
                      (stage.required || stage.present),
                  )
                  .flatMap((stage) => [
                    `musubix4 approval prepare ${stage.stage} --domain ${d.name}`,
                    `musubix4 approval record ${stage.stage} --approver <name> --artifact-sha256 <approved-hash> --domain ${d.name} --confirm`,
                  ]),
              ),
              ...(approvals.release &&
              approvals.release.status !== "approved" &&
              (approvals.release.required || approvals.release.present)
                ? [
                    "musubix4 approval prepare release",
                    "musubix4 approval record release --approver <name> --artifact-sha256 <approved-hash> --confirm",
                  ]
                : []),
            ]
          : approvals.stages
              ...
              .flatMap((stage) => [
                `musubix4 approval prepare ${stage.stage}`,
                `musubix4 approval record ${stage.stage} --approver <name> --artifact-sha256 <approved-hash> --confirm`,
              ])
        : status !== "pass"
          ? ["musubix4 trace build", "musubix4 gate"]
          : [],
  };
}
