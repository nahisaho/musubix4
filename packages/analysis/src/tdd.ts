...
/** @id CODE-AUTONOMOUS-HISTORICAL-TDD-SCOPE-001
 * @implements REQ-AUTONOMOUS-DEVELOPMENT-002
 * @design DES-AUTONOMOUS-DEVELOPMENT-014
 */
export function isHistoricalBaselineRequirement(path: string): boolean {
  return /^baseline-specs\/features\/[^/]+\/requirements\.md$/.test(path);
}
...
export function parseMusubixTestReport(text: string): MusubixTestReport {
  ...
  if (value.schemaVersion !== 1 || !Array.isArray(value.tests))
    throw new Error("Invalid structured TDD report.");
  for (const test of value.tests) {
    ...
    if (
      ...
      (!test.operations ||
        typeof test.operations !== "object" ||
        Array.isArray(test.operations) ||
        Object.entries(test.operations).some(
          ([name, count]) =>
      ...
      throw new Error(
        "Structured test operations must be nonnegative integer counters.",
      );
    }
  }
  if (value.testFiles !== undefined) {
    if (
      ...
      throw new Error("Structured testFiles must be an object.");
    }
    for (const [id, path] of Object.entries(value.testFiles)) {
      if (
        typeof path !== "string" ||
        !path ||
        path.includes("\0") ||
        ...
        throw new Error(
          `Structured test path for ${id} must be a safe relative path.`,
        );
      }
    }
  }
...
function chainRecordSha256(
  record: Omit<TddChainRecord, "recordSha256">,
): string {
  return digest(JSON.stringify(record));
}

function appendChainRecord(
  ...
  if (!cycle.cycleId)
    throw new Error("TDD cycle ID is required for append-only evidence.");
  if (!evidence.chain) {
    if (evidence.cycles.some((entry) => entry !== cycle)) {
      throw new Error(
        "Existing TDD evidence lacks an append-only hash chain; regenerate it before recording new phases.",
      );
    }
    evidence.chain = [];
  }
...
async function sourceFingerprint(
  ...
  const paths = evidenceInputPaths(await files(root)).filter(
    (path) => path !== testPath && !excluded.has(path),
  );
...
async function testFingerprint(root: string, test: TraceNode): Promise<string> {
  ...
  const start =
    lines.slice(0, Math.max(0, test.line - 1)).join("\n").length +
    (test.line > 1 ? 1 : 0);
  if (isSource(test.path)) {
    ...
    const declaration = statements.find(
      (statement) => statement.getStart(source) >= start,
    );
  ...
  const next = text
    ...
    .search(/^[ \t]*(?:\/\*+|\/\/|#).*?@id\s+TEST-/m);
  return digest(
    text.slice(start, next < 0 ? text.length : searchFrom + next).trim(),
  );
}
 ...
 * @design DES-AUTONOMOUS-DEVELOPMENT-001
 */
export async function legacyTestFingerprint(
  root: string,
  test: TraceNode,
): Promise<string> {
  const text = await readText(root, test.path);
...
export async function migrateTddFingerprint(
  root: string,
  testId: string,
  approver: string,
): Promise<TddMigrationResult> {
  if (!approver)
    throw new Error(
      "An approver is required to migrate TDD fingerprint evidence.",
    );
  ...
  if (voidLinkage(evidence, order, cycle).valid) {
    const validlyVoided = new Set(
      evidence.cycles.filter(
        (entry) => voidLinkage(evidence, order, entry).valid,
      ),
    );
    const effective = effectiveLatestCycle(
      evidence,
      order,
      validlyVoided,
      testId,
  ...
  if (!cycle.cycleId)
    throw new Error(
      `${testId} lacks a cycle ID; regenerate its evidence before migrating.`,
    );
  if (!cycle.green?.valid)
    throw new Error(`${testId} has no valid Green phase to migrate.`);
  if (cycle.migrate) throw new Error(`${testId} has already been migrated.`);
  const trace = await buildTrace(root);
  const test = trace.nodes.find(
    (node) => node.kind === "test" && node.id === testId,
  );
  ...
  record.order = (
    await appendEvidenceOrder(root, {
      kind: "tdd",
...
export async function loadTddEvidence(
  root: string,
): Promise<TddEvidence | null> {
  ...
  if (value.schemaVersion !== 1 || !Array.isArray(value.cycles))
    throw new Error("Invalid TDD evidence.");
  return value;
}
...
function phaseLinkageValid(
  ...
  const orderRecord = evidenceOrderRecord(
    order.records,
    "tdd",
    cycle.cycleId,
    phase,
  ...
  return (
    recordSha256 === chainRecordSha256(payload) &&
 ...
 * @implements REQ-AUTONOMOUS-DEVELOPMENT-001, REQ-AUTONOMOUS-DEVELOPMENT-001
 * @design DES-AUTONOMOUS-DEVELOPMENT-001
 */
function voidLinkage(
  ...
  if (phaseLinkageValid(order, evidence.chain, cycle, "void", cycle.void))
    return { valid: true };
  ...
  return {
    valid: false,
    reason: `${cycle.testId}'s void evidence is malformed: ${reason}.`,
  };
}
...
function effectiveLatestCycle(
  ...
  const voidRecord = evidenceOrderRecord(
    order.records,
    "tdd",
    voidedCycle.cycleId,
    "void",
  ...
  for (const candidate of evidence.cycles) {
    ...
    if (
      ...
      continue;
    const record = evidenceOrderRecord(
      order.records,
      "tdd",
      candidate.cycleId,
      "green",
    ...
    if (!best || record.sequence > best.sequence)
      best = { cycle: candidate, sequence: record.sequence };
  }
  return best?.cycle;
}
 ...
 * @implements REQ-AUTONOMOUS-DEVELOPMENT-001, REQ-AUTONOMOUS-DEVELOPMENT-001
 * @design DES-AUTONOMOUS-DEVELOPMENT-001, DES-AUTONOMOUS-DEVELOPMENT-001
 */
export async function voidTddCycle(
  root: string,
  testId: string,
  approver: string,
  reason: string,
): Promise<TddVoidResult> {
  if (!approver?.trim())
    throw new Error("An approver is required to void a TDD cycle.");
  if (!reason?.trim())
    throw new Error("A reason is required to void a TDD cycle.");
  const evidence = await loadTddEvidence(root);
  if (!evidence) throw new Error("No TDD evidence found.");
  const cycle = evidence.cycles
    .filter((entry) => entry.testId === testId)
    .at(-1);
  if (!cycle) throw new Error(`No TDD cycle found for ${testId}.`);
  if (cycle.green?.valid)
    throw new Error(
      `${testId}'s latest cycle has a valid Green phase; only a dangling cycle can be voided.`,
    );
  if (cycle.void)
    throw new Error(`${testId}'s latest cycle is already voided.`);
  if (!cycle.cycleId)
    throw new Error(
      `${testId} lacks a cycle ID; regenerate its evidence before voiding.`,
    );
  ...
  const order = await inspectEvidenceOrder(root);
  const earlierCycles = evidence.cycles
    .slice(0, evidence.cycles.indexOf(cycle))
    .filter((entry) => entry.testId === testId);
  const eligibleFallback = earlierCycles.some(
    (entry) =>
      entry.red.valid &&
      entry.green?.valid &&
      !voidLinkage(evidence, order, entry).valid,
  );
  if (!eligibleFallback) {
    return {
      voided: false,
...
export async function runTddPhase(
  ...
  if (phase === "red") {
    const domain = await resolveRequirementDomain(
      root,
      config.approval,
      requirementId,
    );
  ...
  if ((!command.tddArgs?.length || !command.tddReport) && !command.adapter) {
    throw new Error(
      `Configured command ${commandName} needs tddArgs and a structured tddReport.`,
    );
  }
  if (phase === "red") {
    for (const name of config.tdd.redPreflightCommands) {
      ...
      if (execution.status !== "completed" || execution.exitCode !== 0) {
        throw new Error(
          `TDD Red preflight command ${name} failed with status ${execution.status} and exit code ${execution.exitCode ?? "none"}.`,
        );
      }
    }
  }
  ...
  const evidence: TddEvidence = (await loadTddEvidence(root)) ?? {
    schemaVersion: 1,
    cycles: [],
    chain: [],
  };
  if (
    evidence.cycles.some((cycle) =>
      [cycle.red, cycle.green, cycle.refactor].some(
        (item) => item && !Number.isInteger(item.order),
      ),
    )
  ) {
    throw new Error(
      "Existing TDD evidence lacks monotonic order; regenerate it before recording new phases.",
    );
  }
  ...
  const previous = evidence.cycles
    .filter(
      (cycle) =>
        cycle.testId === testId && cycle.requirementId === requirementId,
    )
    .at(-1);
  if (phase !== "red") {
    if (!previous)
      throw new Error(
        `${phase} must use the same requirement and command as Red.`,
      );
    if (previous.commandName !== commandName)
      throw new Error(
        `${phase} must use the same requirement and command as Red.`,
      );
    if (!previous.red.valid)
      throw new Error(`A valid Red phase is required before ${phase}.`);
    if (previous.red.testFingerprint !== currentFingerprint)
      throw new Error("The test changed after Red; run the Red phase again.");
    if (phase === "refactor" && !previous.green?.valid)
      throw new Error("A valid Green phase is required before Refactor.");
  }
  const adapter = command.adapter
    ? adapterInvocation(command.adapter, command.name, testId, test.path)
    : null;
  ...
  const targetedArgs = command.tddArgs
    ...
    : adapter!.args;
  const configuredArgs = command.args.map((arg) =>
    render(arg, testId, test.path, reportPath),
  );
  ...
  if (adapter) {
    reportText = await readAdapterOutput(
      adapter,
      reportAbsolute,
      execution.stdout,
    );
  ...
  if (reportText === null) {
    diagnostics.push(
      error(
        "TDD_REPORT_MISSING",
        `${testId} did not produce a fresh structured TDD report.`,
        reportPath,
      ),
    );
  }
  if (reportText !== null) {
    try {
      ...
      if (report.tests.length !== 1 || report.tests[0]?.id !== testId) {
        diagnostics.push(
          error(
            "TDD_REPORT_NOT_SCOPED",
            `${testId} must be the only test result in the structured report.`,
            reportPath,
          ),
        );
      } else {
        ...
        if (testStatus !== expectedStatus) {
          diagnostics.push(
            error(
              "TDD_TARGET_RESULT",
              `${phase} requires ${testId} to report ${expectedStatus}, observed ${testStatus}.`,
              reportPath,
            ),
          );
        }
  ...
  const exitValid =
    ...
    (phase === "red" ? execution.exitCode !== 0 : execution.exitCode === 0);
  if (!exitValid)
    diagnostics.push(
      error(
        "TDD_PHASE_RESULT",
        `${phase} requires a completed command with ${expectedExit} exit status.`,
      ),
    );
  if (!Number.isSafeInteger(execution.durationMs) || execution.durationMs < 0) {
    diagnostics.push(
      error(
        "TDD_DURATION_INVALID",
        `${phase} produced an invalid execution duration; archive the invalid evidence and regenerate this cycle from a clean Red baseline.`,
      ),
    );
  }
  const currentSourceFingerprint = await sourceFingerprint(root, test.path, [
    reportPath,
  ]);
  if (
    phase === "green" &&
    previous?.red.sourceFingerprint === currentSourceFingerprint
  ) {
    diagnostics.push(
      error(
        "TDD_GREEN_WITHOUT_SOURCE_CHANGE",
        `${testId} has no non-test project change between Red and Green.`,
        test.path,
      ),
    );
  }
  ...
  if (!cycleId)
    throw new Error(`A cycle ID is required before recording ${phase}.`);
  ...
  if (phase === "red" && evidence.cycles.length === 0) {
    const otherMandatoryRequirements = trace.nodes.filter(
      (node) =>
        node.kind === "requirement" &&
        node.mandatory &&
        node.id !== requirementId,
    );
    const uncoveredIds = otherMandatoryRequirements
      .filter((requirement) => {
        const verifiedTests = trace.edges
          .filter(
            (edge) =>
              edge.relation === "verifies" && edge.to === requirement.id,
          )
          .map((edge) => edge.from);
        return !evidence.cycles.some(
          (cycle) =>
    ...
    const hasChangeDocuments = (await files(root)).some((path) =>
      /^\.musubix\/changes\/CHANGE-\d+\.md$/.test(path),
    );
    const alreadyRequired =
      config.requiredChecks.includes("tdd") || hasChangeDocuments;
...
export async function validateTddEvidence(root: string): Promise<{
  ...
  voided: Array<{
    testId: string;
    cycleId: string;
    void: { approver: string; reason: string; recordedAt: string };
  }>;
}> {
  const evidence = await loadTddEvidence(root);
  ...
  if (!evidence.chain) {
    diagnostics.push(
      error(
        "TDD_CHAIN_MISSING",
        "TDD evidence lacks the append-only hash chain.",
      ),
    );
  } else {
    ...
    for (const [index, record] of evidence.chain.entries()) {
      ...
      const expectedPrevious =
        index === 0 ? null : evidence.chain[index - 1]!.recordSha256;
      const { recordSha256, ...payload } = record;
      if (record.sequence !== expectedSequence) {
        diagnostics.push(
          error(
            "TDD_CHAIN_SEQUENCE",
            `TDD chain record ${record.sequence} is out of order; expected ${expectedSequence}.`,
          ),
        );
      }
      if (record.previousSha256 !== expectedPrevious) {
        diagnostics.push(
          error(
            "TDD_CHAIN_LINK",
            `TDD chain record ${record.sequence} does not link to the preceding record.`,
          ),
        );
      }
      if (recordSha256 !== chainRecordSha256(payload)) {
        diagnostics.push(
          error(
            "TDD_CHAIN_HASH_MISMATCH",
            `TDD chain record ${record.sequence} has an invalid SHA-256.`,
          ),
        );
      }
      ...
      if (records.has(key))
        diagnostics.push(
          error(
            "TDD_CHAIN_PHASE_DUPLICATE",
            `${key} appears more than once in the TDD chain.`,
          ),
        );
    ...
    for (const cycle of evidence.cycles) {
      for (const phase of [
        ...
        if (!record) {
          diagnostics.push(
            error(
              ...
              `${cycle.testId}:${phase} is absent from the TDD hash chain.`,
              cycle.testPath,
            ),
          );
          continue;
        ...
        if (
          record.requirementId !== cycle.requirementId ||
          record.testId !== cycle.testId ||
          ...
          diagnostics.push(
            error(
              "TDD_CHAIN_PAYLOAD_MISMATCH",
              `${cycle.testId}:${phase} does not match its immutable TDD chain record.`,
              cycle.testPath,
            ),
          );
        }
    ...
    for (const record of records.values()) {
      diagnostics.push(
        error(
          "TDD_CHAIN_ORPHAN",
          `TDD chain record ${record.sequence} has no matching cycle phase.`,
        ),
      );
    }
  }
    ...
    for (const cycles of cyclesByTest.values()) {
      for (let index = 0; index < cycles.length; index++) {
        if (
          cycles
            .slice(index + 1)
            .some((later) => later.red.valid && later.green?.valid)
        ) {
  ...
  for (const cycle of evidence.cycles) {
    ...
    if (linkage.valid) {
      ...
      voided.push({
        ...
        cycleId: cycle.cycleId!,
        void: {
          approver: cycle.void.approver,
          reason: cycle.void.reason,
          recordedAt: cycle.void.recordedAt,
        },
      });
    } else {
      diagnostics.push(
        error("TDD_VOID_EVIDENCE_MALFORMED", linkage.reason!, cycle.testPath),
      );
    }
  }
  const trace = await buildTrace(root, false);
  for (const requirement of trace.nodes.filter(
    (node) =>
      node.kind === "requirement" &&
      node.mandatory &&
      !isHistoricalBaselineRequirement(node.path),
  )) {
    const verifiedTests = trace.edges
      .filter(
        (edge) => edge.relation === "verifies" && edge.to === requirement.id,
      )
      .map((edge) => edge.from);
    const covered = evidence.cycles.some(
      (cycle) =>
        cycle.requirementId === requirement.id &&
        verifiedTests.includes(cycle.testId) &&
        cycle.red.valid &&
  ...
  for (const cycle of evidence.cycles) {
    for (const phase of ["red", "green", "refactor"] as const) {
      ...
      if (!Number.isSafeInteger(item.durationMs) || item.durationMs < 0) {
        diagnostics.push(
          error(
            "TDD_DURATION_INVALID",
            `${cycle.testId}:${phase} has an invalid execution duration; archive the invalid evidence and regenerate this cycle from a clean Red baseline.`,
            cycle.testPath,
          ),
        );
      }
      if (!cycle.cycleId || !Number.isInteger(item.order)) {
        diagnostics.push(
          error(
            "TDD_ORDER_MIGRATION_REQUIRED",
            `${cycle.testId}:${phase} lacks monotonic order evidence; archive legacy evidence and regenerate the complete cycle instead of editing append-only records.`,
            cycle.testPath,
          ),
        );
        continue;
      ...
      if (!record || record.sequence !== item.order) {
        diagnostics.push(
          error(
            "TDD_ORDER_MISMATCH",
            `${cycle.testId}:${phase} does not match the monotonic evidence order log.`,
            cycle.testPath,
          ),
        );
      }
    ...
    if (
      cycle.green?.order !== undefined &&
      cycle.red.order !== undefined &&
      cycle.green.order <= cycle.red.order
    ) {
      diagnostics.push(
        error(
          "TDD_ORDER_SEQUENCE",
          `${cycle.testId}:green is not after Red in monotonic evidence order.`,
          cycle.testPath,
        ),
      );
    }
    if (
      cycle.refactor?.order !== undefined &&
      cycle.green?.order !== undefined &&
      ...
      diagnostics.push(
        error(
          "TDD_ORDER_SEQUENCE",
          `${cycle.testId}:refactor is not after Green in monotonic evidence order.`,
          cycle.testPath,
        ),
      );
    }
    if (cycle.migrate) {
      if (!cycle.cycleId || !Number.isInteger(cycle.migrate.order)) {
        diagnostics.push(
          error(
            "TDD_ORDER_MIGRATION_REQUIRED",
            `${cycle.testId}:migrate lacks monotonic order evidence; archive legacy evidence and regenerate the complete cycle instead of editing append-only records.`,
            cycle.testPath,
          ),
        );
      } else {
        ...
        if (!record || record.sequence !== cycle.migrate.order) {
          diagnostics.push(
            error(
              "TDD_ORDER_MISMATCH",
              `${cycle.testId}:migrate does not match the monotonic evidence order log.`,
              cycle.testPath,
            ),
          );
        }
      }
      const latestNonMigrateOrder = cycle.refactor?.valid
        ? cycle.refactor.order
        : cycle.green?.order;
      if (
        cycle.migrate.order !== undefined &&
        latestNonMigrateOrder !== undefined &&
        cycle.migrate.order <= latestNonMigrateOrder
      ) {
        diagnostics.push(
          error(
            "TDD_ORDER_SEQUENCE",
            `${cycle.testId}:migrate is not after Green/Refactor in monotonic evidence order.`,
            cycle.testPath,
          ),
        );
      }
      if (!cycle.migrate.approver) {
        diagnostics.push(
          error(
            "TDD_LEGACY_OR_UNSCOPED_EVIDENCE",
            `${cycle.testId}:migrate lacks a recorded human approver.`,
            cycle.testPath,
          ),
        );
      }
    ...
    if (
      !supersededCycles.has(cycle) &&
      !validlyVoidedCycles.has(cycle) &&
      (!cycle.red.scoped ||
        !cycle.red.resultObserved ||
        cycle.red.testStatus !== "failed" ||
        !cycle.red.reportSha256 ||
        !cycle.red.sourceFingerprint ||
        !cycle.red.executionId)
    ) {
      diagnostics.push(
        error(
          "TDD_LEGACY_OR_UNSCOPED_EVIDENCE",
          `${cycle.testId} lacks test-scoped execution provenance; archive the legacy cycle and regenerate it from a clean Red baseline: move .musubix/evidence/tdd.json aside and re-record every cycle with tdd red/green/refactor. There is no partial prune command; hand-editing the evidence is not supported.`,
          cycle.testPath,
        ),
      );
    }
    if (
      ...
      diagnostics.push(
        error(
          "TDD_RED_MISSING",
          `${cycle.testId} has no valid failing Red phase; archive it and regenerate the complete cycle by moving .musubix/evidence/tdd.json aside and re-recording every cycle.`,
          cycle.testPath,
        ),
      );
    if (
      ...
      diagnostics.push(
        error(
          "TDD_GREEN_MISSING",
          `${cycle.testId} has no valid passing Green phase; archive it and regenerate the complete cycle by moving .musubix/evidence/tdd.json aside and re-recording every cycle.`,
          cycle.testPath,
        ),
      );
    if (cycle.green?.valid) {
      if (
        !cycle.green.scoped ||
        !cycle.green.resultObserved ||
        cycle.green.testStatus !== "passed" ||
        !cycle.green.reportSha256 ||
        !cycle.green.sourceFingerprint ||
        !cycle.green.executionId
      ) {
        diagnostics.push(
          error(
            "TDD_LEGACY_OR_UNSCOPED_EVIDENCE",
            `${cycle.testId} Green lacks test-scoped execution provenance.`,
            cycle.testPath,
          ),
        );
      }
      ...
      const isStaleTarget =
        ...
        (actualLatest !== undefined &&
          validlyVoidedCycles.has(actualLatest) &&
          effectiveLatestCycle(
            evidence,
            order,
            validlyVoidedCycles,
            cycle.testId,
      ...
      if (isStaleTarget) {
        ...
        if (cycle.refactor?.valid && cycle.refactor.order !== undefined) {
          candidates.push({
            order: cycle.refactor.order,
            fingerprint: cycle.refactor.testFingerprint,
          });
        }
        if (cycle.migrate?.order !== undefined)
          candidates.push({
            order: cycle.migrate.order,
            fingerprint: cycle.migrate.toFingerprint,
          });
        const latest = candidates.sort((a, b) => b.order - a.order)[0];
        if (latest && current !== latest.fingerprint)
          diagnostics.push(
            error(
              "TDD_TEST_STALE",
              `${cycle.testId} changed after its latest passing TDD phase.`,
              cycle.testPath,
            ),
          );
      }
      if (cycle.red.sourceFingerprint === cycle.green.sourceFingerprint) {
        diagnostics.push(
          error(
            "TDD_GREEN_WITHOUT_SOURCE_CHANGE",
            `${cycle.testId} has no non-test project change between Red and Green.`,
            cycle.testPath,
          ),
        );
      }
    ...
    if (cycle.green && cycle.green.commandSha256 !== cycle.red.commandSha256) {
      diagnostics.push(
        error(
          "TDD_COMMAND_CHANGED",
          `${cycle.testId} used a different command between Red and Green.`,
          cycle.testPath,
        ),
      );
    }
    if (
      cycle.refactor &&
      cycle.refactor.commandSha256 !== cycle.red.commandSha256
    ) {
      diagnostics.push(
        error(
          "TDD_COMMAND_CHANGED",
          `${cycle.testId} used a different command during Refactor.`,
          cycle.testPath,
        ),
      );
    }
    if (
      cycle.refactor?.valid &&
      (!cycle.refactor.scoped ||
        !cycle.refactor.resultObserved ||
        cycle.refactor.testStatus !== "passed" ||
        !cycle.refactor.reportSha256 ||
        !cycle.refactor.sourceFingerprint ||
        !cycle.refactor.executionId)
    ) {
      diagnostics.push(
        error(
          "TDD_LEGACY_OR_UNSCOPED_EVIDENCE",
          `${cycle.testId} Refactor lacks test-scoped execution provenance.`,
          cycle.testPath,
        ),
      );
    }
  ...
  for (const phase of ["red", "green", "refactor"] as const) {
    ...
    for (const cycle of evidence.cycles) {
      ...
      const key = item.reportSha256
        ? `report:${item.reportSha256}`
        : `output:${item.outputSha256}`;
