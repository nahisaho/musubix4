...
export async function requireApproval(root: string, stage: ApprovalStage, config: ApprovalConfig, domain?: ResolvedDomain): Promise<void> {
  ...
  if (result.status !== 'approved') {
    ...
    throw new Error(`${stage} approval${scope} is ${result.status}; record explicit current approval before continuing. Run \`musubix4 approval validate\` for a full per-stage status.`);
  }
}
