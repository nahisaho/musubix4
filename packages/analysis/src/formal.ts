...
export function generateSmt2(model: Pick<FormalModel, 'literals'> & Partial<Pick<FormalModel, 'constraints'>>): string {
  ...
  return [
    '; Generated by musubix4. This checks only the documented explicit constraint model.',
    '(set-logic QF_UFLIA)',
...
function commandFor(name: 'z3' | 'lean', options: FormalCheckOptions): SolverInvocation[] {
  if (name === 'z3') return [{ name, command: options.z3Command ?? process.env.MUSUBIX4_Z3 ?? 'z3', prefixArgs: [] }];
  const configured = options.leanCommand ?? process.env.MUSUBIX4_LEAN;
  ...
  for (const name of ['z3', 'lean'] as const) {
    ...
    solvers.push({
      ...
      recommendation: status === 'available'
        ...
        : status === 'missing'
          ? name === 'z3'
            ? 'Install Z3 or configure --z3-command / MUSUBIX4_Z3.'
            : 'Install Lean with elan and ensure lean or lake is available, or configure --lean-command / MUSUBIX4_LEAN.'
          : status === 'timeout'
