...
export async function traceInputs(root: string): Promise<string[]> {
  ...
  try {
    ...
    isMusubixRepository = packageManifest.name === 'musubix4';
  } catch {
    // Consumer projects do not need to trace musubix4's own Skill definitions.
  }
...
export async function buildTrace(root: string, persist = true): Promise<TraceGraph> {
  ...
  for (const path of paths) {
    ...
    } else if (/^\.musubix\/decisions\/ADR-\d+\.md$/.test(path)) {
...
export async function loadTrace(root: string): Promise<TraceGraph> {
  ...
  if (!path) throw new Error('Trace graph missing; run musubix4 trace build.');
