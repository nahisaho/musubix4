...
export function githubOidcAudience(
  ...
  if (keyBinding === 'key-id') return `${audience}#musubix4-key-id=${encodeURIComponent(keyId)}`;
  ...
  return `${audience}#musubix4-key-sha256=${createHash('sha256').update(der).digest('base64url')}`;
}
