...
type ExplicitRequirement = Omit<Requirement, "formal"> & {
...
function traceHash(trace: TraceGraph): string {
  return digest(
    canonical({
      nodes: trace.nodes.map(({ id, kind, path, line, mandatory }) => ({
        id,
        kind,
        path,
        line,
...
function testProvenanceHash(
  test: Omit<CorrespondenceTestEvidence, "provenanceSha256">,
): string {
...
async function explicitRequirements(
  root: string,
): Promise<ExplicitRequirement[]> {
  const result: ExplicitRequirement[] = [];
  for (const path of (await files(root)).filter((entry) =>
    /^\.musubix\/features\/[^/]+\/requirements\.md$/.test(entry),
  )) {
    result.push(
      ...validateRequirements(await readText(root, path), path)
        .value.filter(
          (
            requirement,
          ): requirement is Omit<Requirement, "formal"> & {
            formal: FormalConstraint;
          } => requirement.formal !== null,
        )
...
export async function writeModelCorrespondenceEvidence(
  ...
  const entries = requirements.map((requirement): ModelCorrespondenceEntry => {
    const linked = new Set(
      trace.edges
        .filter(
          (edge) => edge.to === requirement.id && edge.relation === "verifies",
        )
        .map((edge) => edge.from),
    );
    const tests = executions.flatMap((execution) =>
      execution.tests
        .filter(
          (test) =>
            linked.has(test.id) &&
            test.status === "passed" &&
            execution.processStatus === "completed" &&
            execution.exitCode === 0,
        )
        .flatMap((test): CorrespondenceTestEvidence[] => {
          const node = trace.nodes.find(
            (candidate) =>
              candidate.kind === "test" && candidate.id === test.id,
          );
          if (!node) return [];
          const provenance: Omit<
            CorrespondenceTestEvidence,
            "provenanceSha256"
          > = {
            testId: test.id,
            testPath: node.path,
          ...
          return [
            { ...provenance, provenanceSha256: testProvenanceHash(provenance) },
          ];
        }),
    );
    return {
...
export function modelCorrespondenceEvidenceHead(
...
async function currentReport(
  ...
  const invocation = command.adapter
    ? adapterInvocation(command.adapter, command.name)
    : null;
  ...
  const configuredArgs = command.args.map((arg) =>
    arg.replaceAll("{reportPath}", configuredPath),
  );
  ...
  return {
    commandSha256: digest(
      canonical({ executable: command.command, arguments: args }),
    ),
    ...
    testFiles: Object.fromEntries(
      Object.entries(report.testFiles ?? {}).map(([id, path]) => [
        id,
        path.replaceAll("\\", "/").replace(/^\.\//, ""),
      ]),
    ),
    pathBindingRequired: Boolean(command.testReport),
  };
}

export async function validateModelCorrespondenceEvidence(
  root: string,
): Promise<{
  present: boolean;
  valid: boolean;
        ...
        ? [
            error(
              ...
              "Requirements with explicit Formal JSON require model correspondence evidence. " +
                "Run `npx musubix4 evidence refresh` to generate .musubix/evidence/model-correspondence.json.",
              path,
            ),
          ]
        : [],
  ...
  try {
    ...
    ) as ModelCorrespondenceEvidence;
    formalEvidence = JSON.parse(
      await readText(root, ".musubix/evidence/formal.json"),
    ) as FormalEvidenceShape;
  } catch (cause) {
    return {
      present: true,
      valid: false,
      ...
      diagnostics: [
        error(
          "MODEL_CORRESPONDENCE_SCHEMA",
          `Invalid correspondence/formal evidence: ${cause instanceof Error ? cause.message : String(cause)}.`,
          path,
        ),
      ],
    };
  ...
  if (
    ...
    formalEvidence.schemaVersion !== 1 ||
    !formalEvidence.result ||
    !formalEvidence.fingerprints
  ) {
    return {
      present: true,
      valid: false,
      ...
      diagnostics: [
        error(
          "MODEL_CORRESPONDENCE_SCHEMA",
          "Invalid model correspondence evidence.",
          path,
        ),
      ],
    };
  ...
  try {
    ...
    if (!checkedTrace.valid)
      diagnostics.push(
        error(
          "MODEL_CORRESPONDENCE_TRACE_STALE",
          "Generated trace evidence is missing, stale, or invalid.",
          path,
        ),
      );
  } catch {
    return {
      ...
      diagnostics: [
        error(
          "MODEL_CORRESPONDENCE_TRACE_STALE",
          "Generated trace evidence is missing or invalid.",
          path,
        ),
      ],
    };
  ...
  if (value.traceEvidenceSha256 !== currentTraceHash) {
    diagnostics.push(
      error(
        "MODEL_CORRESPONDENCE_TRACE_STALE",
        "Correspondence trace fingerprint does not match the current generated trace.",
        path,
      ),
    );
  }
  if (value.formalEvidenceSha256 !== formalEvidenceHash(formalEvidence)) {
    diagnostics.push(
      error(
        "MODEL_CORRESPONDENCE_FORMAL_TAMPERED",
        "Correspondence formal evidence fingerprint is invalid.",
        path,
      ),
    );
  }
  const requirementFingerprints = await snapshot(
    root,
    requirements.map((requirement) => requirement.path),
  );
  for (const requirement of requirements) {
    if (
      formalEvidence.fingerprints[requirement.path] !==
      requirementFingerprints[requirement.path]
    ) {
      diagnostics.push(
        error(
          "MODEL_CORRESPONDENCE_FORMAL_STALE",
          `${requirement.id} formal input fingerprint is stale.`,
          requirement.path,
          requirement.line,
        ),
      );
    }
    const modeled = formalEvidence.result.constraints.filter(
      (entry) => entry.requirement === requirement.id,
    );
    if (
      modeled.length !== 1 ||
      canonical(modeled[0]?.constraint) !== canonical(requirement.formal)
    ) {
      diagnostics.push(
        error(
          "MODEL_CORRESPONDENCE_MODEL_STALE",
          `${requirement.id} has no unique current formal model entry.`,
          requirement.path,
          requirement.line,
        ),
      );
  ...
  for (const entry of value.entries) {
    if (
      ...
      !validRelativePath(entry.requirementPath) ||
      !/^[a-f0-9]{64}$/.test(entry.formalSha256) ||
      !/^[a-f0-9]{64}$/.test(entry.modelSha256) ||
      !/^[a-f0-9]{64}$/.test(entry.traceSha256) ||
      !Array.isArray(entry.tests)
    ) {
      diagnostics.push(
        error(
          "MODEL_CORRESPONDENCE_SCHEMA",
          "A correspondence entry is malformed.",
          path,
        ),
      );
      continue;
    ...
    if (seen.has(entry.requirementId)) {
      diagnostics.push(
        error(
          "MODEL_CORRESPONDENCE_DUPLICATE",
          `${entry.requirementId} has duplicate correspondence entries.`,
          path,
        ),
      );
    }
    ...
    if (!requirement || requirement.path !== entry.requirementPath) {
      diagnostics.push(
        error(
          "MODEL_CORRESPONDENCE_REQUIREMENT",
          `${entry.requirementId} is not a current requirement with explicit Formal JSON.`,
          path,
        ),
      );
      continue;
    ...
    if (
      entry.formalSha256 !== formalHash(requirement.formal) ||
      entry.modelSha256 !== modelHash(requirement.id, requirement.formal) ||
      entry.traceSha256 !== currentTraceHash
    ) {
      diagnostics.push(
        error(
          "MODEL_CORRESPONDENCE_STALE",
          `${entry.requirementId} model or trace fingerprint is stale.`,
          path,
        ),
      );
    }
    ...
    for (const test of entry.tests) {
      if (
        ...
        !validRelativePath(test.testPath) ||
        !/^[a-f0-9]{64}$/.test(test.testSha256) ||
        typeof test.commandName !== "string" ||
        !/^[a-f0-9]{64}$/.test(test.commandSha256) ||
        !validRelativePath(test.reportPath) ||
        ...
        diagnostics.push(
          error(
            "MODEL_CORRESPONDENCE_SCHEMA",
            `A test evidence entry for ${entry.requirementId} is malformed.`,
            path,
          ),
        );
        continue;
      ...
      if (testProvenanceHash(provenance) !== provenanceSha256) {
        diagnostics.push(
          error(
            "MODEL_CORRESPONDENCE_PROVENANCE_TAMPERED",
            `${test.testId} correspondence provenance was changed.`,
            path,
          ),
        );
        continue;
      ...
      const node = trace.nodes.find(
        (candidate) =>
          candidate.id === test.testId && candidate.kind === "test",
      );
      if (
        !node ||
        node.path !== test.testPath ||
        !trace.edges.some(
          (edge) =>
            edge.from === test.testId &&
            edge.to === requirement.id &&
            edge.relation === "verifies",
        )
      ) {
        diagnostics.push(
          error(
            "MODEL_CORRESPONDENCE_TEST_UNLINKED",
            `${test.testId} is not an authoritative trace-linked test for ${requirement.id}.`,
            path,
          ),
        );
        continue;
      }
      const fingerprint = (await snapshot(root, [test.testPath]))[
        test.testPath
      ];
      if (fingerprint !== test.testSha256) {
        diagnostics.push(
          error(
            "MODEL_CORRESPONDENCE_TEST_STALE",
            `${test.testId} fingerprint is stale.`,
            test.testPath,
          ),
        );
        continue;
      ...
      try {
        ...
        if (
          !report ||
          report.commandSha256 !== test.commandSha256 ||
          report.reportSha256 !== test.reportSha256 ||
          !report.passed.has(test.testId) ||
          (report.pathBindingRequired &&
            report.testFiles[test.testId] !==
              test.testPath.replaceAll("\\", "/").replace(/^\.\//, ""))
        ) {
          diagnostics.push(
            error(
              "MODEL_CORRESPONDENCE_TEST_NOT_PASSED",
              `${test.testId} lacks a current passing structured command report.`,
              test.reportPath,
            ),
          );
          continue;
        ...
        diagnostics.push(
          error(
            "MODEL_CORRESPONDENCE_TEST_NOT_PASSED",
            `${test.testId} report is missing, malformed, or stale.`,
            test.reportPath,
          ),
        );
        continue;
    ...
    if (!validTests) {
      diagnostics.push(
        error(
          "MODEL_CORRESPONDENCE_UNPROVED",
          `${requirement.id} has no current authoritative passing test evidence.`,
          path,
        ),
      );
  ...
  for (const requirement of requirements) {
    if (!seen.has(requirement.id)) {
      diagnostics.push(
        error(
          "MODEL_CORRESPONDENCE_UNPROVED",
          `${requirement.id} has no correspondence entry.`,
          requirement.path,
          requirement.line,
        ),
      );
