...
import { parseHohConfig, type HohConfig } from './hoh.js';
...
export function parseConfig(input: unknown): Config {
  ...
  keys(value, ['schemaVersion', 'language', 'qualityProfile', 'commands', 'requiredChecks', 'thresholds', 'architecture', 'codeGraph', 'formal', 'mutation', 'tdd', 'approval', 'workflow', 'attestation', 'hoh'], 'config');
...
export async function loadConfig(root: string): Promise<Config> {
  ...
  if (!await exists(within(root, path))) throw new Error('Missing .musubix/config.json; run musubix4 init.');
...
export function parsePolicyBaseline(input: unknown): PolicyBaseline {
  ...
  const requiredCommands = strings(optional(value.requiredCommands, []), 'requiredCommands');
  const commandNames = new Set(parsed.commands.map((command) => command.name));
  for (const name of requiredCommands) {
    if (!commandNames.has(name)) {
      throw new Error(`policy baseline requiredCommands entry ${name} has no commands definition.`);
    }
  }
  return {
    schemaVersion: 1,
...
export function policyDiagnostics(config: Config, baseline: PolicyBaseline, changed: string[] | null = null): Diagnostic[] {
  ...
  for (const name of baseline.requiredCommands) {
    const configuredCommand = config.commands.find((command) => command.name === name);
    if (!configuredCommand?.required) {
  ...
  for (const baselineCommand of baseline.commands) {
    const configuredCommand = config.commands.find((command) => command.name === baselineCommand.name);
    if (configuredCommand && JSON.stringify(configuredCommand) !== JSON.stringify(baselineCommand)) {
      diagnostics.push(error(
        'POLICY_COMMAND_DEFINITION',
        `Command ${baselineCommand.name} differs from the trusted baseline definition.`,
        baselineCommand.name,
      ));
    }
  }
  if (baseline.commands.length) {
    const allowedCommands = new Set(baseline.commands.map((command) => command.name));
    for (const command of config.commands) {
      if (!allowedCommands.has(command.name)) {
        diagnostics.push(error(
          'POLICY_COMMAND_UNLISTED',
          `Command ${command.name} is not present in the trusted baseline allowlist.`,
          '.musubix/config.json',
        ));
      }
    }
