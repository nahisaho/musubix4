  ...
  return {
    ...
    project: {
      preflight: async ({ config }: { config: HohConfig }) => {
        if (!config.model || config.budget.aiCredits <= 0) throw new Error('A fixed model and positive AI-credit budget are required.');
        if (!Object.keys(config.commands).length) throw new Error('At least one project command is required.');
      ...
      mandatoryChecks: async ({
        ...
        run: { config: HohConfig };
        workspace?: { path: string };
      }) => runMandatoryProjectChecks({
        cwd: workspace?.path ?? root,
        commands: validateMandatoryQaConfiguration(run.config),
        timeoutMs: run.config.commandTimeoutMs,
        maxOutputBytes: run.config.maxCommandOutputBytes,
        policy: derivePolicy('qa', run.config.permissions.qa ?? {}),
      }),
      candidateChecks: async ({ run }: { run: { config: HohConfig } }) => {
        const policy = derivePolicy('developer', run.config.permissions.developer ?? {});
        for (const argv of Object.values(run.config.commands)) {
    ...
    git: {
      ...
      createQaWorkspace: async ({ run, candidate }: { run: { id: string; config: HohConfig }; candidate: { ref: string; treeDigest: string } }) => {
      ...
      cleanupQaWorkspace: async ({ run, workspace }: { run: { id: string }; workspace: { path: string } }) => {
        await candidates.get(run.id)?.cleanupQaWorkspace(workspace);
      },
      treeDigest: async ({ run, candidate, workspace }: {
        run: { id: string; config: HohConfig };
        candidate: { ref: string; treeDigest: string };
        workspace?: { path: string };
      }) => {
        const adapter = candidates.get(run.id) ?? new GitCandidateStore(
          root,
    ...
    github: {
      loadIssue: async (source) => {
        const policy = derivePolicy('planner', {});
        const result = await runBoundedProcess({
          argv: ['gh', 'issue', 'view', String(source.issue), '--repo', source.repository, '--json', 'body,title,url,id'],
          cwd: root,
          timeoutMs: 30_000,
          maxOutputBytes: 1_048_576,
          policy,
        });
        if (result.exitCode !== 0) throw new Error(`gh issue view failed: ${result.stderr}`);
        const issue = JSON.parse(result.stdout) as { body: string; title: string; url: string; id: string };
        return { locator: `${issue.url}@${issue.id}`, content: `# ${issue.title}\n\n${issue.body}` };
      },
    },
  };
}
...
export function createProgram(): Command {
  ...
  const version = readPackageVersion(resolve(packageRoot, 'package.json'));
  const program = new Command().name('musubix4').description('Evidence-driven SDD for GitHub Copilot CLI / 根拠に基づく仕様駆動開発').version(version);
  ...
  common(program.command('upgrade').description('Refresh bundled skill files to match the installed musubix4 version; never touches config, constitution, or feature artifacts'))
  ...
  common(formal.command('check <file>'))
    ...
    .option('--z3-command <path>', 'Z3 executable path (or MUSUBIX4_Z3)')
    .option('--lean-command <path>', 'Lean executable path (or MUSUBIX4_LEAN)')
  ...
  common(
    correspondence.command('validate')
      ...
      .description('Validate model correspondence evidence (run `npx musubix4 evidence refresh` first to generate .musubix/evidence/model-correspondence.json)'),
  ).action(async (options: { root: string; json?: boolean }) => {
  ...
  common(approval.command('record <stage>'))
    ...
    .option('--domain <name>', 'Approval domain (required when approval.domains is configured, except for release)')
    .option('--run <run-id>', 'Bind this decision to one paused MUSUBIX4 HoH run')
    .option('--nonce <paused-state-nonce>', 'Current paused-state nonce for --run')
    .action(async (stage: string, options: {
      root: string; json?: boolean; approver: string; artifactSha256: string; confirm: boolean; domain?: string; run?: string; nonce?: string;
    }) => {
      ...
      const root = resolve(options.root);
      if (options.run || options.nonce) {
        if (!options.run || !options.nonce) throw new Error('--run and --nonce must be provided together.');
        const evidence = await recordRunApproval(new FileRunStore(root), {
          runId: options.run,
          stage,
          nonce: options.nonce,
          manifestDigest: options.artifactSha256,
          approver: options.approver,
        });
        output(evidence, !!options.json, `Recorded run-bound ${stage} approval for ${options.run}.`);
        return;
      }
      const config = await loadConfig(root);
  ...
  common(program.command('run').description('Start a durable MUSUBIX4 Harness-of-Harness run'))
    .option('--prompt <text>', 'Public specification prompt')
    .option('--markdown <file>', 'Public Markdown specification')
    .option('--musubix <file>', 'Existing .musubix requirements file')
    ...
    .action(async (options: {
      ...
      const sources = [
        ...
        options.markdown === undefined ? null : { kind: 'markdown', path: options.markdown },
        options.musubix === undefined ? null : { kind: 'musubix', path: options.musubix },
        options.githubIssue === undefined ? null : (() => {
          const match = options.githubIssue.match(/^([^#]+)#(\d+)$/);
          if (!match) throw new Error('--github-issue must use owner/repo#number.');
          return { kind: 'github-issue', repository: match[1]!, issue: Number(match[2]) };
        })(),
      ].filter(Boolean) as SpecificationSource[];
      if (sources.length !== 1) throw new Error('Specify exactly one of --prompt, --markdown, --musubix, or --github-issue.');
      const loaded = await loadConfig(root);
      const standalone = await loadHohConfigFile(root);
      const raw = standalone ?? loaded.hoh ?? (options.model && options.budget
        ? parseHohConfig({
          model: options.model,
          budget: { aiCredits: Number(options.budget) },
          commands: Object.fromEntries(loaded.commands.map((command) => [command.name, [command.command, ...command.args]])),
        })
        : undefined);
  ...
  common(protectedSet.command('amend <run-id>').description('Prepare a collision-inventory amendment for requirements approval'))
    .requiredOption('--collision-inventory <path>', 'Repository-relative replacement collision inventory')
    .action(async (
      runId: string,
      ...
      let run = current.state === 'approval-paused'
        && current.approval?.stage === 'requirements'
        && current.amendmentManifest?.path === options.collisionInventory
        ? current
        : current.state === 'amendment-required'
          && current.approval?.stage === 'requirements'
          && current.approval.decision === 'approved'
          && current.amendmentManifest?.path === options.collisionInventory
          ? await applyApprovedCollisionInventoryAmendment(
            store,
          ...
          : await store.prepareCollisionInventoryAmendment(runId, options.collisionInventory);
      if (run.state === 'approval-paused' && await exists(within(root, '.musubix/hoh.json'))) {
        const services = localHohServices(root, store);
        if (!services.roles.reviewer) throw new Error('Verified automatic approval requires a Reviewer role.');
        run = await autoApproveRunBoundary(store, runId, services.roles.reviewer);
        if (
          run.state === 'amendment-required'
          && run.approval?.decision === 'approved'
          && run.approval.automated
        ) {
          run = await applyApprovedCollisionInventoryAmendment(
            store,
            new GitCandidateStore(
              root,
              runId,
      ...
      output(
        ...
        !!options.json,
        run.state === 'candidate'
          ? `Run ${run.id}: collision inventory amendment promoted`
          : `Run ${run.id}: requirements approval required for ${run.amendmentManifest?.sha256 ?? 'unknown digest'}`,
      );
    });
  ...
  common(program.command('stop <run-id>').description('Request a resumable stop for a MUSUBIX4 run'))
    .action(async (runId: string, options: { root: string; json?: boolean }) => {
      const run = await new FileRunStore(resolve(options.root)).stop(runId, 'cancelled');
      output(run, !!options.json, `Run ${run.id}: stopped`);
    });
